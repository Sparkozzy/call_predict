...
### [2026-05-22] — Falha no spawn de Servidores MCP no Windows por caminhos com espaços
- **Contexto:** Inicialização de servidores MCP locais (como o Supabase MCP) gerenciados por engines baseadas em Electron/Node.js no Windows.
- **Erro:** O servidor MCP falhava silenciosamente e não registrava nenhuma ferramenta na sessão do chat de IA.
- **Causa raiz:** A engine spawner de MCPs do cliente tenta iniciar o comando diretamente (`shell: false`). Caminhos absolutos contendo espaços no nome do diretório (como `C:\Program Files\nodejs\npx.cmd`) quebram a quebra de argumentos e falham ao spawnar o executável.
- **Solução:** Substituir caminhos absolutos com espaços por chamadas diretas a comandos globais mapeados no `PATH` do SO (por exemplo, usar apenas `"npx"` no campo `"command"`).
- **Lição:** No Windows, para servidores baseados em Node.js ou ferramentas do sistema, prefira sempre o uso de comandos simples resolvidos globalmente pelo `PATH` (como `npx`, `python`, `node`) no `mcp_config.json`, evitando caminhos absolutos com espaços.
